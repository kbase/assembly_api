...
def _get_data_from_ws(ws, ref, included):
...
def get_assembly_id(ws, ref):
    return ws.get_object_info3({'objects': [{'ref': ref}]})['infos'][0][1]


def get_genome_annotations(ws, ref):
...
def get_number_contigs(workspace_client, ref):
    return _get_data_from_ws(workspace_client, ref, ['num_contigs'])['num_contigs']


def get_gc_content(workspace_client, ref):
    return _get_data_from_ws(workspace_client, ref, ['gc_content'])['gc_content']


def get_dna_size(workspace_client, ref):
    return _get_data_from_ws(workspace_client, ref, ['dna_size'])['dna_size']


def get_contig_ids(workspace_client, ref):
    included = ['contigs/*/contig_id']
    data = _get_data_from_ws(workspace_client, ref, included)
    return [x for x in data.get('contigs', {})]


def get_contig_lengths(workspace_client, ref, contig_id_list):
    included = [f'contigs/{cid}/length' for cid in contig_id_list]
    data = _get_data_from_ws(workspace_client, ref, included)
    return {k: data['contigs'][k].get('length') for k in contig_id_list}


def get_contig_gc_content(workspace_client, ref, contig_id_list):
...
def get_contigs(workspace_client, ref, contig_id_list):
